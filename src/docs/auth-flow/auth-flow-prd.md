...
**2025 Native Integration Pattern:**
As of April 1, 2025, Clerk is officially supported as a third-party authentication provider in Supabase. This native integration eliminates the need for custom JWT templates and simplifies the setup process.

**User actions (manual steps):**

**Step 1: Configure Clerk for Supabase Compatibility**
- [ ] Go to [Clerk Dashboard](https://dashboard.clerk.com/)
- [ ] Navigate to **Integrations** → **Connect with Supabase**
- [ ] Follow Clerk's setup wizard to configure your Clerk instance for Supabase compatibility
- [ ] Copy your Clerk instance domain (you'll need this for Supabase)
- [ ] Keep the Clerk dashboard tab open - you'll need API keys from here

**Step 2: Set Up Supabase Third-Party Auth**
- [ ] Go to [Supabase Dashboard](https://app.supabase.com/)
- [ ] Select your ATJ-ERP project
- [ ] Navigate to **Authentication** → **Sign In/Up** → **Third Party Auth**
- [ ] Click **Add new third-party authentication provider**
- [ ] Select or add **Clerk** as the provider
- [ ] Enter your Clerk instance domain (from Step 1)
...
Add these redirect URLs (no need to copy from anywhere - use the new fallback naming):
...
Ask me to paste these values into your .env.local file - I'll help format them correctly but you'll add them to the file.
...
function Header() {
  return (
    <header className="flex items-center justify-between px-6 py-4 border-b">
      <h1 className="text-xl font-bold">ATJ-ERP</h1>
      <SignedIn>
        <UserButton />
      </SignedIn>
      <SignedOut>
...
**Note:** Use Tailwind classes to match your design system instead of inline styles.
...
Based on the codebase, the main protected route is `/dashboard`. Additional protected routes include:
...
- `/hr` - HR module
- `/audit` - Audit module
- `/admin` - Admin module (if user has permissions)

The middleware pattern `/dashboard(.*)` will protect all dashboard routes and sub-routes.
...
- ✅ No JWT template needed (native integration handles this)
- ✅ Use `accessToken()` function instead of custom fetch wrapper
...
If you're developing locally with Supabase CLI or self-hosting, add this configuration to your `supabase/config.toml` file:
...
Replace `your-clerk-domain.clerk.accounts.dev` with your actual Clerk instance domain from the Clerk Dashboard.
...
**Supabase Integration (2025 Native Integration):**
- **Native Third-Party Auth:** As of April 1, 2025, Clerk is officially supported as a third-party authentication provider in Supabase
- **No JWT Templates Needed:** The native integration eliminates the need for custom JWT templates or sharing Supabase JWT secrets
- **Simplified Setup:** Configure Clerk in Supabase dashboard under **Authentication > Sign In/Up > Third Party Auth**
- **Two-Step Setup:**
  1. Configure Clerk for Supabase compatibility (Clerk Dashboard → Integrations)
  2. Add Clerk as third-party auth provider (Supabase Dashboard → Authentication → Third Party Auth)
- **Supabase Client Pattern:** Use `accessToken()` function to pass Clerk's session token (see Stage 5 for code examples)
- **RLS Policies:** Use `auth.jwt()->>'sub'` to get Clerk user ID from session token
- **No Webhooks Required:** Basic auth + data isolation works through tokens, no webhook sync needed
- **Deprecated Method:** Previous JWT template method is deprecated as of April 1, 2025
- **Benefits:**
  - Enhanced security (no JWT secret sharing)
  - Improved performance (no token fetching per request)
